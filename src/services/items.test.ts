...
describe('Item service', () => {
    ...
    (fetcher as jest.Mock).mockResolvedValue(new Promise((resolve) => resolve({})));
	
		await itemsFetch('test');

		expect(fetcher).toHaveBeenCalled();
		expect(fetcher).toHaveBeenCalledWith('http://localhost:8080/api/items?q=test');
	});
    ...
    (fetcher as jest.Mock).mockResolvedValue(new Promise((resolve) => resolve({})));
		...
		expect(fetcher).toHaveBeenCalled();
		expect(fetcher).toHaveBeenCalledWith('http://localhost:8080/api/items/1234');
	});
});